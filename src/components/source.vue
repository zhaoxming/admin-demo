  ...
  <div class="">
      ...
      <div class="list">
        <div style="float:right">
          <span> 共有<span style="color:#ff5a5f">{{houseListSize}}</span>条数据</span>
          <span class="nextButton" style="padding:5px;">申请上架</span>
          <span class="nextButton" style="padding:5px;background:#aaa">申请下架</span>
          <span class="nextButton" style="padding:5px;background:#aaa">批量删除</span>
        </div>
      </div>

      <div class="list" style="marginTop:-20px;height:100px;">
          <Table :loading="loading" border ref="selection" :columns="houseCols" :data="houseList" @on-select="selectItem"
        ...
        houseCols: [
            ...
            },
            {
                ...
                render: (h, params) => {
                    ...
                    const hot = params.row.isHot === '1' ? '是' : '否';
                    return h('div', [
                        h('strong', hot)
                    ]);
                }
            },
            {
                ...
                render: (h, params) => {
                    ...
                    const houseStatus = params.row.status == '0' ? '编辑中'
                        : params.row.status == '1' ? '申请上架'
                        : params.row.status == '2' ? '已上架'
                        : params.row.status == '3' ? '上架失败'
                        : params.row.status == '4' ? '申请下架'
                        : params.row.status == '5' ? '已下架'
                        : params.row.status == '5' ? '下架失败'
                        :'编辑中';
                    return h('div', [
  ...
  mounted() {
    ...
    methods: {
      getHouseList:function () {
          let getUrl = SERVER_ROOT_URL+'/house/admin/get_description_list?managerId='+sessionStorage.getItem('userId')
              +'&page=0&pageSize=0';
          this.$http.get(getUrl
              ...
              if(res.body.status === 200){
                  ...
                  this.houseListSize = this.houseList.length;
        ...
        delete:function (index) {
            let getUrl = SERVER_ROOT_URL+'/house/admin/delete_house?id='+this.houseList[index].id;
            this.$http.get(getUrl
            ).then(function (res) {
                console.log(res);
                if(res.body.status === 200){
